...
const SkillBadge = memo(({ skill, index }) => {
  ...
  return (
    <span
      ref={ref}
      className="px-5 py-2.5 rounded-full border-2 border-primary/40 text-sm font-medium text-foreground/90 hover:border-primary hover:bg-primary/10 hover:scale-110 hover:shadow-xl hover:shadow-primary/40 transition-all duration-300 cursor-default group overflow-hidden relative flex items-center gap-2"
    >
      <FontAwesomeIcon
        icon={skill.icon}
...
export const PremiumAboutSection = ({ showImage = true }) => {
  ...
  // Border glow animation — opacity-only (no boxShadow repaint)
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          ref={orb1Ref}
          className="absolute -top-40 -right-40 w-96 h-96 bg-gradient-to-br from-primary/20 to-purple-500/10 rounded-full blur-xl"
        />
        <div
          ref={orb2Ref}
          className="absolute -bottom-40 -left-40 w-96 h-96 bg-gradient-to-tr from-purple-500/15 to-primary/10 rounded-full blur-xl"
        />
        <div
          ref={orb3Ref}
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-gradient-to-r from-primary/10 to-accent/10 rounded-full blur-xl"
        />
        <BackgroundParticles />
